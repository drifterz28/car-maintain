...
import { FormControl, InputLabel, MenuItem, Select, Modal, Typography, Button, Box } from '@mui/material';
...
const AddVehicle: React.FC<AddVehicleProps> = ({ open, setOpen }) => {
  ...
  const handleChange = (e: unknown) => {
    ...
    if (name === 'year' && formData.year !== value) {
      ...
      const isClean =
        [formData.make, formData.model, formData.year].every(Boolean) &&
  ...
  useEffect(() => {
    ...
    } else if (formData.model && carData[formData.make][formData.model].length === 1) {
      setIsClean(true);
      setFormData({
        ...formData,
        bodyStyle: carData[formData.make][formData.model][0],
      });
    }
  ...
  const saveCar = (e) => {
    e.preventDefault();
    // @ts-expect-error not a dispatchable event
    ...
    >
      <Box sx={style}>
        ...
        <form onSubmit={saveCar}>
            ...
            <>
              ...
              <FormControl fullWidth>
                ...
                >
                  ...
                  {bodyStyle.map((style: string, i) => (
                    <MenuItem key={i} value={style.trim()}>
                      {style.trim()}
                    </MenuItem>
                  ))}
                </Select>
          ...
          <Box mt={2}>
            <Button variant="contained" disabled={!isClean} type="submit">
              Add Car
            </Button>
          </Box>
        </form>
